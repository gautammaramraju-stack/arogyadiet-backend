...
function assignFallbackDriverForOrder(order, allOrders, allDrivers) {
  const franchiseId = order.franchiseId || 1;
  const drivers = (allDrivers || []).filter(d => (d.franchiseId || 1) == franchiseId && (d.status || 'active') !== 'deleted' && (d.status === 'active' || d.status === 'available' || d.status === 'busy'));
  if (!drivers.length) return null;
  const sameDate = (allOrders || []).filter(o => o.delivery_date === order.delivery_date && o.driverId);
  const counts = {};
  sameDate.forEach(o => { counts[o.driverId] = (counts[o.driverId] || 0) + 1; });
  const chosen = drivers
    .map(d => ({ d, c: counts[d.id] || 0 }))
    .sort((a, b) => a.c - b.c)[0];
  ...
  app.post('/api/subscriptions', (req, res) => {
    ...
    // Hard fallback: ensure immediate assignment for testing/demo flow if any order is still unassigned
    data = loadData();
    ...
    createdOrders.forEach(ord => {
      ...
      if (idx < 0) return;
      if (allOrders[idx].driverId) return;
      const fallbackDriver = assignFallbackDriverForOrder(allOrders[idx], allOrders, allDrivers);
      if (!fallbackDriver) return;
      allOrders[idx].driverId = fallbackDriver.id;
      allOrders[idx].driverName = fallbackDriver.name || '';
      allOrders[idx].driverPhone = fallbackDriver.phone || '';
      allOrders[idx].status = allOrders[idx].status === 'delivered' ? 'delivered' : 'assigned';
      changed = true;
    });
    if (changed) {
